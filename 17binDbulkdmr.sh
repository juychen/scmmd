...
output_dir="/data2st2/junyi/output/atac1112/tobiasbam/BULK/BIND_DMRJ"
...
for bam_file in "$folder1"/*footprints.bw; do
    ...
    if [[ -f "$folder2/$new_name" ]]; then
        ...
        if [[ ! -f "$output_dir/finish_list/$filename.txt" ]]; then
            ...
            # /data1st2/hannan_25/data/Nanopore_processV1/nanopore_08_differential/summary/dmr_sets/DMR_pycis_pipeline/pycistarget_out/motif_use/v10nr_clust.from_hdf5.meme
