...
for (ctstatus in unique(df_deg$ctstatus)) {
  ...
  if (file.exists(paste(outdir, "CP_enrichment",ctstatus,".csv",sep = "_")) && !overwrite) {
    print(paste("File already exists:", paste(outdir, "CP_enrichment",ctstatus,".csv",sep = "_")))
    next
  }
    tryCatch(
    {
      ...
      if(length(gene_list) > 0) {
        ...
        gene_entrez <- bitr(unique(gene_list), 
                    fromType = "SYMBOL", 
                    toType = "ENTREZID", 
                    OrgDb = "org.Mm.eg.db")
        ...
        ego <- enrichGO(gene = entrez_ids,
                        ...
                        pAdjustMethod = "BH",  # Adjust p-values using Benjamini-Hochberg method (similar to g_SCS)
                        pvalueCutoff = 0.05,
                        qvalueCutoff = 0.05,
        ...
        if(!is.null(ego) && nrow(ego) > 0) {
          ...
          res = subset(res, p.adjust<=0.05)
          res$nlog10p <- -log10(res$p.adjust)
          ...
          write.csv(res, file = file.path(outdir,paste0("CP_enrichment_",ctstatus,".csv")), row.names = FALSE)
...
if (length(all_results) > 0) {
  ...
  write.csv(full_results, file = file.path(outdir,paste0("CP_enrichment_",region,"_all.csv")), row.names = FALSE)
  print(paste("Full results saved to", file.path(outdir,paste0("CP_enrichment_",region,"_all.csv"))))
} else {
