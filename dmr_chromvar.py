...
def prepare_input_adata(adata: ad.AnnData, args: argparse.Namespace) -> ad.AnnData:
    """
    Normalize the input to a 'count' layer for chromVAR.
    
    Strategy:
    - If `adata.layers['count']` already exists, use it as raw counts.
    - Else, treat the current `adata.X` as raw counts and store a copy in
      `adata.layers['count']` so downstream chromVAR code can find it.
    - The matrix is sparse counts (peaks x cells), no need to renormalize
      because chromVAR only needs raw counts to compute GC-bias and motifs.
    """
    adata = adata.copy()

    # Ensure a 'count' layer exists
    if "count" not in adata.layers:
        print("  No 'count' layer found, treating adata.X as raw counts ...")
    ...
    # backup total counts to obs 
    if "total_counts" in adata.obs:
        adata.obs["total_counts_full"] = adata.obs["total_counts"].copy()
    ...
    mu.pp.filter_obs(adata, "total_counts_full", lambda values: values >= args.min_total_counts)
