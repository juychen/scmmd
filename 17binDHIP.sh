...
folder1="/data2st2/junyi/output/atac0627/tobiasbam/MC_HIP/corrected/"
folder2="/data2st2/junyi/output/atac0627/tobiasbam/MW_HIP/corrected/"
...
for bam_file in "$folder1"/*footprints.bw; do
    ...
    if [[ -f "$folder2/$new_name" ]]; then
        ...
        if [[ ! -f "$output_dir/finish_list/$filename.txt" ]]; then
            ...
            --outdir /data2st2/junyi/output/atac0627/tobiasbam/HIP/ --cond_names MC MW --cores 32 \
        ...
        # If finish the write a text to '/data2st2/junyi/output/atac0627/tobiasbam/MW_HIP/finish_list'
    else
