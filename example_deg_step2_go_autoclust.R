...
heatmap <- function(
  ...
  if (!is.null(row)) {
    # remove GO terms whose Class is in excluded (keep only terms not in excluded)
    if (group_col %in% names(row)) {
      # filter by the dynamic group_col (e.g., "Class") against names(excluded)
      row <- row %>% filter(!(.data[[group_col]] %in% names(excluded)))
    }
  ...
  # Do not split rows by module/class; keep automatic clustering of rows
  row_split <- NULL
