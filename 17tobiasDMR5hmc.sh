  ...
  for file in $folder/*; do
    ...
    if [[ $file == *.bam ]]; then
      ...
      if [[ -f $out_bam ]]; then
        ...
        mkdir -p $folder/correct_5hmc
        #cat "test" > $folder/corrected/$sample_name\_$ctname.txt
        out_bw=$folder/correctcorrect_5hmc_dmr/$ctname\_corrected.bw
        ...
        if [[ ! -f $out_bw ]]; then
          ...
          --peaks /data2st1/junyi/output/atac1112/cCRE/5hmC_${ctname:0:3}.bed \
          --blacklist /data2st1/junyi/ref/mm10-blacklist.v2.bed \
          --outdir $folder/correct_5hmc \
          --cores 64
        fi
        # else print exist
        out_fp=$folder/correct_5hmc/$ctname\_footprints.bw
        if [[ ! -f $out_fp ]]; then
          echo "Output BW file $out_fp not exists. calculating..."
          echo "generating footprints $out_fp"
          TOBIAS FootprintScores --signal $folder/correct_5hmc/$ctname\_corrected.bw \
          --regions /data2st1/junyi/output/atac1112/cCRE/5hmC_${ctname:0:3}.bed \
          --output $out_fp \
          --cores 64
        fi
      fi
