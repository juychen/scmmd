...
def run_aggregate(args):
	...
	logger.info("Calculating footprint depth measure (improved)")
	logger.info("FPD (signal,regions): footprint_width baseline middle FPD osc")

	# 可调参数（建议先用默认）
	SHOULDER_W = 5          # baseline shoulder width (bp)
	CONTIG_BP = 3           # 连续低谷窗口（bp）
	OSC_PAD = 5             # 在中心两侧额外取多少bp算oscillation
	...
	for row, signal_name in enumerate(signal_names):
		for col, region_name in enumerate(region_names):
			...
			# 建议保证起始宽度至少覆盖 motif 一半，且不要太小
			fp_start = max(int(motif_width / 2), 4)  # fp_flank 是 half-width
			fp_end = min(25, args.flank - SHOULDER_W - 1)

			for fp_flank in range(fp_start, fp_end):

				center = args.flank
				mid_l = center - fp_flank
				mid_r = center + fp_flank

				# -----------------------
				# 1) Shoulder baseline
				# -----------------------
				left_l = mid_l - SHOULDER_W
				left_r = mid_l
				right_l = mid_r
				right_r = mid_r + SHOULDER_W

				# 边界检查
				if left_l < 0 or right_r > len(agg):
					continue

				baseline = np.mean(np.concatenate([agg[left_l:left_r], agg[right_l:right_r]]))

				# -----------------------
				# 2) Middle: deepest contiguous k-bp mean inside motif window
				# -----------------------
				middle_window = agg[mid_l:mid_r]
				if len(middle_window) < CONTIG_BP:
					continue

				# 连续低谷：取最小的连续 CONTIG_BP 平均
				contig_means = [
					np.mean(middle_window[i:i+CONTIG_BP])
					for i in range(0, len(middle_window) - CONTIG_BP + 1)
				]
				middle = float(np.min(contig_means))

				# -----------------------
				# 3) Footprint depth (keep your sign convention)
				# -----------------------
				depth = middle - baseline  # footprint应为负（中心更低）

				# -----------------------
				# 4) Oscillation score (2nd-diff variance around center window)
				# -----------------------
				# 二阶差分更敏感于锯齿
				second_diff = np.diff(np.diff(agg))
				# second_diff 比 agg 少2长度，需要对齐索引
				osc_l = max(0, mid_l - OSC_PAD - 2)
				osc_r = min(len(second_diff), mid_r + OSC_PAD - 2)
				osc = float(np.var(second_diff[osc_l:osc_r])) if (osc_r > osc_l) else np.nan

				FPD_results.append([fp_flank * 2, baseline, middle, depth, osc])

			if len(FPD_results) == 0:
				logger.stats(f"FPD ({signal_name},{region_name}): no valid windows")
				continue

			# 计算稳定性（depth随窗口变化的波动）
			depths = np.array([r[3] for r in FPD_results], dtype=float)
			depth_std = float(np.std(depths))

			# 你可以在这里加一条“自动筛选”的判据（可选）
			# 例：只标记“稳定+非oscillatory+有中心抑制”的窗口
			# 先给出一个保守默认阈值，你可按数据分布调整
			OSC_THR = 0.5
			STABLE_THR = 0.2

			# 仅用于标记，不强制过滤（避免误杀）
			for result in FPD_results:
				fpw, base, mid, dep, osc = result
				tag = ""
				if dep < 0 and osc < OSC_THR and depth_std < STABLE_THR:
					tag = "*"  # 形态更像“可解释 footprint”
				logger.stats(
					"FPD ({0},{1}): {2:2d} {3:.5f} {4:.5f} {5:.5f} osc={6:.5f} std={7:.5f} {8}".format(
						signal_name, region_name, int(fpw), base, mid, dep, osc, depth_std, tag
					)
				)
